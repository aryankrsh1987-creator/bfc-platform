...
export default function AuthPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      setMessage(
        "Check your email for a confirmation link! You can also log in with Google below.",
      );
      setLoading(false);
    } else {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black text-white flex items-center justify-center px-6">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white/5 border border-white/10 rounded-[40px] p-10 backdrop-blur-2xl shadow-[0_0_100px_rgba(168,85,247,0.15)]">
          {/* Title */}
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white text-black px-6 py-4 rounded-2xl font-bold hover:scale-[1.02] transition duration-300 disabled:opacity-50"
          >
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-purple-500 to-blue-500 py-4 rounded-2xl font-black text-lg hover:scale-[1.02] transition duration-300 disabled:opacity-50"
            >
          ...
          {/* Message */}
          {message && (
            <div className="mt-6 bg-purple-500/10 border border-purple-500/20 rounded-2xl p-4 text-purple-300 text-sm">
              {message}
            </div>
          )}
