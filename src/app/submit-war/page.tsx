...
export default function SubmitWarPage() {
  ...
  const submitWar = async () => {
    ...
    // VALIDATION
    if (!crewA.trim() || !crewB.trim() || !winner.trim()) {
      alert("Fill in crew names and winner.");

      return;
    }

    if (crewA.trim() === crewB.trim()) {
      alert("Crew A and Crew B must be different.");

      return;
    }

    if (winner.trim() !== crewA.trim() && winner.trim() !== crewB.trim()) {
      alert("Winner must be either Crew A or Crew B.");

      return;
    }

    if (images.length === 0) {
      alert("Upload at least 1 winning screenshot.");

      return;
    }

    if (opponentScore < 0) {
      alert("Opponent score cannot be negative.");

      return;
    }

    if (opponentScore >= images.length) {
      alert(
        `Winner score (${images.length}) must be greater than opponent score (${opponentScore}).`,
      );
    ...
    // AUTO SCORE
    const autoScore = `${images.length}-${opponentScore}`;

    // SAVE TO DATABASE
    const { error } = await supabase.from("war_submissions").insert([
      {
        crew_a: crewA,
        ...
        submitted_by: authData.user.email,
    ...
    alert(`War submitted! Auto score: ${autoScore}`);
  };
      ...
      <div className="relative z-10 w-full max-w-3xl bg-white/[0.04] border border-white/10 rounded-[32px] p-10 backdrop-blur-xl">
        ...
        <div className="space-y-5">
          <input
            placeholder="Crew A"
            value={crewA}
            onChange={(e) => setCrewA(e.target.value)}
          ...
          <input
            placeholder="Crew B"
            value={crewB}
            onChange={(e) => setCrewB(e.target.value)}
            className="w-full bg-black/30 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-red-500/40"
          />
          ...
          <div className="bg-black/30 border border-dashed border-white/20 rounded-3xl p-8 text-center">
            ...
            <p className="text-zinc-500 mb-6">1 screenshot = 1 point</p>
          ...
          <div className="bg-red-500/10 border border-red-500/20 rounded-3xl p-6">
            <p className="text-red-400 font-bold mb-2">AUTO DETECTED SCORE</p>
          ...
          >
            {uploading ? "Uploading..." : "Submit Verified War"}
          </button>
        </div>
      </div>
    </main>
